    ...
    /* Read in ten chars one by one, encrypt them, write to mem */
    int store_spot = 0x1B; // #30: offset for mem locations to store chars
    ...
    /* Write private key to r1 and address of encrypted string to r2. 
       Special cases required when one of the operands is R0 */
    if (r1 != R0 && r2 != R0) {
